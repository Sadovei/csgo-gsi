...
import { LeftSideOverlay } from '../../types/overlays/left-side'
...
export default function leftSide(
  ...
  let objOverlay: LeftSideOverlay = {
    side: sides?.leftSide ? sides.leftSide.side : '',
    utility: allGrenades,
    economy: {
      roundLose: sides?.leftSide ? (sides.leftSide.side === 'CT'
        ? map.team_ct.consecutive_round_losses
        : map.team_t.consecutive_round_losses) : 0,
      bonus: sides?.leftSide ? (sides.leftSide.side === 'CT'
        ? (map.team_ct.consecutive_round_losses > 5 ? 3400
          : bonus[map.team_ct.consecutive_round_losses])
        : (map.team_t.consecutive_round_losses > 5 ? 3400
  ...
  sides?.leftSide && Object.keys(sides.leftSide.players).forEach((playerSteam) => {
